...
float AnimateStep = 0.00001f;         // Rotation step per update
...
void drawScene(void)
{
	...
	glTranslatef(1, 1, 0.0);               // Translate rotation center from origin
	glRotatef(CurrentAngle, 0.0, 0.0, 1.0);      // Rotate through animation angle
	glTranslatef(-1, -1, 0.0);            // Translate rotation center to origin
	...
	for (int i = 0; i < polygons.size(); i++)
	{
		...
		{
			glVertex3f(polygons[i].points[j].x, polygons[i].points[j].y,  0);
		}
		glEnd();
		k = k == 3 ? 0 : k;
	}
...
int main(int argc, char** argv)
{
	for (int i = -2; i <= 4; i++)
	{
		polygons.push_back(Polygon(Point(i, i), Point(i + 1, i + 1)));
	}
	glutInit(&argc, argv);
